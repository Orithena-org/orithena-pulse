...
def generate_digest(
    ...
    high_signal = [s for s in passed if s.high_signal]
    ...
    sections = [
        ...
        f"**Top Signal:** {n_top} | "
        f"**High Signal (Fit \u22657):** {n_high_signal}",
        "",
    ]

    # High Signal section (fit_score >= 7) — shown first
    if high_signal:
        sections.append("## \U0001f525 High Signal")
        for scored in sorted(high_signal, key=lambda s: s.fit_score, reverse=True):
            fit = scored.fit or {}
            fit_score = fit.get("fit_score", 5)
            product = fit.get("product", "neither")
            why = fit.get("why_unique", "")
            sketch = fit.get("implementation_sketch", "not applicable")
            line = (
                f"- **[Fit {fit_score}/10]** "
                f"[{scored.item.title}]({scored.item.url}) "
                f"({scored.item.source}) — {product}"
            )
            if why:
                line += f"\n  - *Why unique:* {why}"
            if sketch and sketch != "not applicable":
                line += f"\n  - *Sketch:* {sketch}"
